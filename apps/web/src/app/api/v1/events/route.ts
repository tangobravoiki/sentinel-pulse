...
function acToPulseEvent(ac: AdsbAircraft): PulseEvent | null {
  ...
  return {
    ...
    description: [
      ...
      ac.t ? `Type: ${ac.t}` : '',
      `Alt: ${altStr}`,
      ac.gs ? `Speed: ${Math.round(ac.gs)} kt` : '',
      ac.track ? `Track: ${Math.round(ac.track)}°` : '',
      ac.squawk ? `Squawk: ${ac.squawk}` : '',
      ac.emergency && ac.emergency !== 'none' ? `⚠️ Emergency: ${ac.emergency}` : '',
    ]
      .filter(Boolean)
      .join('
'),
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    let events: PulseEvent[] = aircraft
      .map(acToPulseEvent)
